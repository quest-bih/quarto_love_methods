...
It is recommended that you use RStudio to work through this file. To work on your own copy of the project, you can clone it from this repository. There are links to different useful resources throughout the notebook. Ideally you should bookmark them and return to them after the workshop, if you want to dive deeper into the topic.
...
```{r}
#| echo: true
power_calculation <- pwrss.t.2means(mu1 = 30, mu2 = 25,
                                    sd1 = 12, sd2 = 8, kappa = 1, power = .80,
...
This is another one of the essential pieces of information that belongs in the Methods section. One useful way of communicating this is with a Directed Acyclic Graph (DAG). Here we use an example from a previous publication [@lederer2019], which we replicated with the R library `ggdag` [@ggdag]. We can upload the DAG as an image file, as in @fig-dag-image. To enable cross-referencing figures, we gave the figure a unique ID `#fig-dag-image` (Figure IDs have their own prefix `fig-`).

[![Directed Acyclic Graph reproduced from a previously published study.](_manuscript/notebooks/Heavy_computations_files/figure-html/fig-dag-1.png){#fig-dag-image alt="Directed Acyclic Graph reproduced from a previously published study."}](https://dagitty.net/learn/index.html)
...
![This flowchart was taken from a published protocol, uploaded once again as a simple image.](inst/extdata/flowchart.jpg){#fig-flowchart fig-align="left"}
